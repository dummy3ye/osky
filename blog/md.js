...
function renderMarkdown(src) {
    ...
    while (i < lines.length) {
        ...
        if (line.trim().startsWith('|')) {
            ...
            html += '<table>\n<thead>\n<tr>\n';
            cells(tbl[0]).forEach(function(c) { html += '<th>' + inline(c) + '</th>\n'; });
            html += '</tr>\n</thead>\n<tbody>\n';
            for (var t = 1; t < tbl.length; t++) {
                var row = tbl[t].trim();
                if (!row.replace(/[|\s:-]/g, '')) continue;
                html += '<tr>\n';
                cells(row).forEach(function(c) { html += '<td>' + inline(c) + '</td>\n'; });
                html += '</tr>\n';
            }
            html += '</tbody>\n</table>\n';
            continue;
...
function highlight(code, lang) {
    ...
    if (lang === 'c' || lang === 'h') {
        keywords = /\b(if|else|for|while|do|return|break|continue|switch|case|default|goto|sizeof|typedef|struct|enum|union|static|const|extern|void|inline|volatile|register)\b/g;
        types = /\b(int|float|double|char|long|short|unsigned|signed|size_t|FILE|NULL)\b/g;
        commentLine = /\/\/.*/g;
        commentBlock = /\/\*[\s\S]*?\*\//g;
    } else if (lang === 'py' || lang === 'python') {
        keywords = /\b(if|elif|else|for|while|break|continue|return|def|class|import|from|as|try|except|finally|raise|with|yield|lambda|pass|del|global|nonlocal|assert|and|or|not|in|is)\b/g;
        types = /\b(None|True|False|self|int|float|str|list|dict|tuple|set|bool|print|len|range|open|super|type)\b/g;
        commentLine = /#.*/g;
        commentBlock = null;
    } else if (lang === 'sh' || lang === 'bash' || lang === 'shell') {
        keywords = /\b(if|then|else|elif|fi|for|while|do|done|case|esac|in|function|return|exit|local|export|set|unset|shift|break|continue)\b/g;
        types = null;
        commentLine = /#.*/g;
        commentBlock = null;
    } else {
    ...
    while (remaining.length > 0) {
        ...
        if ((lang === 'sh' || lang === 'bash' || lang === 'shell' || !lang) && remaining.match(/^\$/)) {
        ...
        if (wordMatch) {
            var w = wordMatch[0];
            var cls = null;
            if (keywords && w.match(keywords)) cls = 'hl-kw';
            else if (types && w.match(types)) cls = 'hl-type';

            // check if followed by ( for function call
            if (!cls && remaining.charAt(w.length) === '(') cls = 'hl-fn';

            if (cls) tokens.push('<span class="' + cls + '">' + w + '</span>');
            else tokens.push(w);
