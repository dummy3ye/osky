...
function loadProject(owner, repo) {
    ...
    fetch(api)
        ...
        .then(function(d) {
            ...
            var parts = [];
            if (d.stargazers_count === 1) parts.push('1 star');
            else if (d.stargazers_count > 1) parts.push(d.stargazers_count + ' stars');
            if (d.forks_count === 1) parts.push('1 fork');
            else if (d.forks_count > 1) parts.push(d.forks_count + ' forks');
            if (d.language) parts.push(d.language);
            if (meta) meta.textContent = parts.join(' / ');

            var link = document.getElementById('proj-link');
            if (link && d.html_url) link.href = d.html_url;
        })
        .catch(function() {});
    ...
    fetch(api + '/readme')
        ...
        .then(function(d) {
            ...
            md = md.replace(/!\[([^\]]*)\]\((?!https?:\/\/)([^)#]+)(#[^)]*)?\)/g, function(_, alt, url, hash) {
